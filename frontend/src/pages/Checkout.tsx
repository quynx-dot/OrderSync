...
// Note: the Stripe redirect flow calls the backend directly (which returns a
// checkout URL), so the Stripe JS SDK is not needed here. No loadStripe import.
...
const CheckoutPage = () => {
  ...
  const payWithRazorpay = async () => {
    try {
      ...
      if (!order) { setLoadingRazorpay(false); return; }
  ...
  const payWithStripe = async () => {
    ...
    try {
      ...
      if (!order) { setLoadingStripe(false); return; }
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-6 space-y-6">
      ...
      <div className="rounded-xl bg-white p-4 shadow-sm">
        ...
        <p className="text-sm text-gray-600">{restaurant.autoLocation.formattedAddress}</p>
      </div>
          ...
          addresses.map((add) => (
            <label
              ...
              className={`flex gap-3 border p-3 rounded-lg cursor-pointer transition mb-2 ${
                selectedAddressId === add._id ? "border-[#e23744] bg-red-50" : "border-gray-300"
              }`}
            >
              <input
                type="radio"
