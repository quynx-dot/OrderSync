      ...
      <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-sm text-center space-y-4">
        ...
        {paymentId && (
          <div className="rounded-lg">
            <span className="text-sm text-white">Payment ID:</span>
            <p className="text-gray-300 break-all font-mono">{paymentId}</p>
          </div>
        )}
        <div className="space-y-2 pt-2">
          <button
            className="flex w-full items-center justify-center gap-2 bg-[#e23744] py-3 px-4 text-sm text-white font-semibold rounded-lg"
            onClick={() => navigate("/")}
          >
            Continue Shopping <BsArrowRight size={16} />
          </button>
          <button
            className="flex w-full items-center justify-center gap-2 border border-gray-300 py-3 px-4 text-sm text-gray-700 rounded-lg"
