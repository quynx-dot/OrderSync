    ...
    useEffect(() => {
        const verifyPayment = async () => {
            try {
                ...
                // FIX: Refresh cart so count goes back to 0 after successful order
                await fetchCart();
                setVerified(true);
            } catch (error) {
            ...
            <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-sm text-center space-y-4">
                {verified ? (
                    <>
                        ...
                        <p className="text-sm text-gray-500">Your order has been placed and the restaurant has been notified.</p>
                    </>
                ) : (
                    <>
                        <h1 className="text-2xl font-bold text-red-500">Verification Failed</h1>
                        <p className="text-sm text-gray-500">We couldn't verify your payment. Please contact support.</p>
                    </>
                )}
